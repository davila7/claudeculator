  ...
  return (
    <section className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-5">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--color-fg-muted)]">{title}</h3>
        {action}
      </div>
      {children}
    </section>
...
function ProfileSlider({ axis, value, onChange }: ProfileSliderProps) {
  ...
  return (
    <div className="py-2">
      <div className="mb-1 flex items-baseline justify-between">
        <span className="text-xs font-medium">{AXIS_TITLES[axis]}</span>
...
export default function Calculator() {
  ...
  return (
    <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 px-4 py-8 lg:grid-cols-[1fr_300px] lg:px-6">
      {/* LEFT — main form */}
      <div className="flex flex-col gap-4">
        ...
        <Section title="Usage profile">
          ...
          />
          <div className="mt-2 text-xs text-[var(--color-fg-muted)]">{USAGE_PROFILES[config.usage].tagline}</div>
        </Section>

        <Section title="Permissions">
          <div className="mb-4">
            <div className="mb-1 text-xs text-[var(--color-fg-muted)]">defaultMode</div>
            <Segmented
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <div className="mb-2 font-mono text-xs" style={{ color: "var(--color-success)" }}>
                allow
              </div>
              <ChipList
            ...
            <div>
              <div className="mb-2 font-mono text-xs" style={{ color: "var(--color-danger)" }}>
                deny
              </div>
              <ChipList
          ...
          <div className="mt-4">
            <div className="mb-2 font-mono text-xs" style={{ color: "var(--color-warning)" }}>
              ask
            </div>
            <ChipList
          ...
          <div className="mt-3 border-t border-[var(--color-border)] pt-1">
            <Toggle
              ...
              description="Rejects --dangerously-skip-permissions."
        ...
        <Section title="Sandbox">
          <Toggle
            ...
            description="Isolate bash from filesystem & network (macOS / Linux / WSL2)."
          ...
          {config.alwaysThinkingEnabled ? (
            <div className="mt-2">
              <div className="mb-1 flex items-center justify-between text-xs">
                <span className="text-[var(--color-fg-muted)]">MAX_THINKING_TOKENS (env)</span>
        ...
        <div className="flex flex-col items-start gap-2 pt-2">
          <button
            ...
            className="w-full rounded-md bg-[var(--color-fg)] px-5 py-3 text-sm font-semibold text-[var(--color-bg)] transition hover:opacity-90 sm:w-auto sm:min-w-[240px]"
          >
        ...
        {generated ? (
          <div ref={resultRef} className="fadein flex flex-col gap-3 pt-4">
      ...
      {/* RIGHT — compact presets sidebar */}
      <aside className="flex flex-col gap-4 lg:sticky lg:top-20 lg:h-fit">
        <Section title="Presets">
          {AXIS_ORDER.map((axis) => (
            <ProfileSlider key={axis} axis={axis} value={scores[axis]} onChange={(lvl) => updateAxis(axis, lvl)} />
          ))}
        </Section>

        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-4">
          <div className="text-[10px] uppercase tracking-wider text-[var(--color-fg-muted)]">Monthly estimate</div>
          <div className="mt-1 font-mono text-2xl font-medium">{formatUSD(scores.cost.costPerMonthUSD)}</div>
          <div className="mt-1 text-xs text-[var(--color-fg-muted)]">
            {formatNumber(scores.cost.tokensPerMonth)} tokens · {formatNumber(scores.cost.tokensPerSession)} / session
          </div>
          {scores.cost.cacheSavingsPercent > 0 ? (
            <div className="mt-1 text-xs" style={{ color: "var(--color-success)" }}>
              −{scores.cost.cacheSavingsPercent}% cache savings
            </div>
          ) : null}
        </div>
      </aside>
